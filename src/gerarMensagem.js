...
var resultado = document.getElementById('criptografada');
var adicionar = document.getElementById('add');
var alfa2 = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z'];
...
function alterarLetra(letra, chave) {
    if (chave === void 0) { chave = 0; }
    var posicao = alfa2.indexOf(letra);
    if (posicao + chave > 25) {
...
function criptografarPalavra() {
    ...
    var esconderChave = document.getElementById('esconderChave').value != 2 && document.getElementById('esconderChave').value != 3 ? 2 : Number(document.
        getElementById('esconderChave').value);
    var chave2 = document.getElementById('chave').value == '' || document.getElementById('chave').value > 26 || document.getElementById('chave').value < 1 ? 1 : Number(document.getElementById('chave').value);
    // let [palavra, esconderChave, chave2] = getConfig()
    // let [palavra, esconderChave, chave2] = ['ola', 3, 1]//a
    var letraOnde = pegarLetraOnde(esconderChave);
    var letraChave = alfa2[chave2 - 1];
    ...
    if (esconderChave == 2) {
        palavra = "".concat(letraOnde).concat(letraChave).concat(palavraCripto);
    }
    else {
        var separada = palavraCripto.split('');
        palavra = '';
        for (var i = 0; i < separada.length + 2; i++) {
            if (i == 0) {
                palavra += letraOnde;
            } //onde esta a chave
            else if (i == 1) {
...
adicionar.onclick = function () {
    var resultPrevio = resultado.value;
    var palavra = criptografarPalavra();
    resultado.innerText = "".concat(resultPrevio, " ").concat(palavra);
    console.log(resultPrevio + palavra);
    resultado.value = contPalavra ? "".concat(resultPrevio, " ").concat(palavra) : palavra;
    contPalavra++;
};
